    ...
    <div class="card max-w-2xl mx-auto">
        ...
        <form method="POST" action="{{ route('payments.update', $payment) }}">
            ...
            <div class="mb-4">
                ...
                <input type="number" step="0.01" name="amount" id="amount" class="form-input" value="{{ old('amount', $payment->amount) }}" required>
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="transaction_id" id="transaction_id" class="form-input" value="{{ old('transaction_id', $payment->transaction_id) }}">
            </div>
            
            <div class="mb-4">
                <label for="payment_date" class="block text-gray-700 text-sm font-bold mb-2">Payment Date</label>
                <input type="date" name="payment_date" id="payment_date" class="form-input" value="{{ old('payment_date', $payment->payment_date ? $payment->payment_date->format('Y-m-d') : date('Y-m-d')) }}" required>
            </div>
            
            <div class="mb-6">
                <label for="notes" class="block text-gray-700 text-sm font-bold mb-2">Notes</label>
                <textarea name="notes" id="notes" class="form-input" rows="3">{{ old('notes', $payment->notes) }}</textarea>
            </div>
